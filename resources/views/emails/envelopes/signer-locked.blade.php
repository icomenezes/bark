...
<div class="wrap">
  ...
  <div class="body">
    <h2>Link de assinatura bloqueado</h2>
    <p>O link de <strong>{{ $signer->name }}</strong> ({{ $signer->email ?? $signer->whatsapp }}) em
       <strong>{{ $envelopeModel->title }}</strong> foi bloqueado após
       {{ \App\Models\EnvelopeSigner::MAX_CPF_ATTEMPTS }} tentativas com um CPF diferente do informado no envio.</p>
    <div class="box">
      <p>Isso acontece quando quem abriu o link não é a pessoa esperada — ou quando o CPF
         cadastrado no envio está errado.</p>
    </div>
    <p>Confira o cadastro e, se estiver tudo certo, desbloqueie o link na página do documento.</p>
    <p><a class="btn" href="{{ route('envelopes.show', $envelopeModel) }}">Abrir documento</a></p>
  </div>
  <div class="footer">
